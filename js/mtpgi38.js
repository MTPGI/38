...
function handleTouchOrHover(event) {
  ...
  if (event.type == 'mouseover' && $id('qr-code').classList.contains('visible')) {
    return false
  }
...
$id('container').addEventListener('scroll', function() {
  ...
  if (this.scrollTop > window.innerHeight) { 
    toTopClassList.add('visible')
  } else {
	toTopClassList.remove('visible')
  }
});
...
function showQRCode(artOrCard, isCard) {
  if ($id('qr-code').classList.contains('visible')) {
	  hideQRCode();
	  return
  }
  ...
  $id('qr-code').classList.add('visible');
}
function hideQRCode() {
  let img = $id('qr-code').querySelector('img');
  img && img.remove();
  $id('qr-code').classList.remove('visible');
}
