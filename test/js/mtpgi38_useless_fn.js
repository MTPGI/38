...
const 
	...
	encodeLinks = () => {
		const encodeAllCharacters = str => Array.from(str).map(char => "%" + char.charCodeAt(0).toString(16).toUpperCase()).join("");
		document.querySelectorAll("a.email, a.tel").forEach(lnk => {
			const spl = lnk.href.split(":");
			lnk.href = spl[0] + ":" + encodeAllCharacters(spl[1]);
		});
	};
...
/*function goSearch() {
	const
		...
		selectedOption = $q(`#searchlist option[value="${input.value}"]`);
	if (selectedOption) {
	  const 
		dataType = selectedOption.dataset.type,
...
function toClipboard(text) {
	const copylistener = event => {
		...
		if (event.clipboardData) {
			event.clipboardData.setData("text/plain", text.innerText);
			event.clipboardData.setData("text/html", text.innerHTML);
...
function generateVCARDtext(artOrCard, isCard) {
	...
	var feed;
	if (isCard) {
		const cardContent = artOrCard.$q("h3, .adr, .url, .tel, .email, .facebook, .twitter, .instagram, .linkedin, .youtube");
		...
		if (firstCard) {
			...
			parentArticle.querySelectorAll(".card").forEach(card => {
				const cardContent = card.$q("h3, .adr, .url, .tel, .email, .facebook, .twitter, .instagram, .linkedin, .youtube");
			...
			feed = parentArticle.$q("h2, h2 p, h3, .adr, .url, .tel, .email, .facebook, .twitter, .instagram, .linkedin, .youtube");
...
/*document.addEventListener("keydown", (event) => {
	if (event.keyCode === 27 || event.key === "Escape") getAway();
	if ((event.keyCode === 13 || event.key === "Enter") && event.target.id == "input--search") {
		goSearch();
	}
});*/